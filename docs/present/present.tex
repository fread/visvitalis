...
\newcommand{\extraslide}{\textcolor{orange}{$\warning$}}
...
\begin{frame}
  \frametitle{\extraslide{} OK, vielleicht nicht ganz so simplel}
...
\begin{frame}
  \frametitle{\extraslide{} Auftritt Zuse}

  \begin{columns}[T]
    \begin{column}{.50\textwidth}
      \begin{tabular}{ccc@{\hskip 2em}c@{\hskip 2em}cc}
        \toprule
        $A$ & $B$ & $C_{\text{in}}$ & \uncover<2->{$A \oplus B$} & $A + B + C_{\text{in}}$ & $C_{\text{out}}$ \\
        \midrule
        ...
        1 & 1 & 0 & \uncover<2->{0} & \alt<3->{$C_{in}$}{0}      & \alt<4->{$A$}{1} \\
...
\begin{frame}[c]
  \frametitle{\extraslide{} Doch mit Details}
...
\begin{frame}
  ...
  Außerdem:
  \begin{itemize}
  \item Programme Laden
  \item Debugging
  \item Speichermonitor
  \end{itemize}
...
\begin{frame}
  \frametitle{\extraslide{} Assembler}
...
\begin{frame}
  \frametitle{\extraslide{} Plus/Minus}
...
\begin{frame}
  ...
  Gerät ist im Nebenraum aufgebaut.

  \bigskip

  \begin{description}[ca. 20:00:]
  \item[Jetzt:] Q\&A
  \item[19:00:] Pause, Abendessen
  \item[ca. 20:00:] Assembler-Workshop
  \end{description}
\end{frame}
...
\begin{frame}
  ...
  \begin{center}
    \texttt{https://github.com/fread/visvitalis}

    \bigskip

    \qrcode[height=0.5\textheight]{https://github.com/fread/visvitalis}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Eine Instruktion pro Zeile
  \item (Mnemonic/Opcode) + Argument\\
    \texttt{~~lai 17}\\
    \texttt{~~addi 6}
  \item Werte definieren und benutzen (\texttt{\#define})\\
    \texttt{~~.equ x 2~~~~~~~~\# Adresse einer Variable}\\
    \texttt{~~.equ answer 42~~\# Konstante}\\
    \texttt{~~lai answer}\\
    \texttt{~~sta x~~~~~~~~~~~\# x = 42}\\
  \item Label vor Instruktion mit Doppelpunkt\\
    Expandiert zu Adresse der Instruktion\\
    \texttt{~~loop: subi 1}\\
    \texttt{~~~~~~~~jmp loop}
  \item Alles ein Namensraum
  \end{itemize}
\end{frame}
